...
class Select extends HTMLElement {
    ...
    async #query(graph, context) {
        ...
        return (await new Comunica.QueryEngine().queryBindings(this.#script.textContent, comunicaContext)).toArray()
    }

    async #instantiate(graph, quads, template, parent, stack) {
        for (const quad of quads) {
            const isCycle = Select.#isCycle(stack, quad)
            let q = quad
            if (isCycle) {
                const t = "mark"
                // const t="throw"
...
customElements.define("rdf-select", Select)
