...
class UriNode extends HTMLElement {
    async initialize(graph, context, stack) {
        ...
        if (this.dataset.template) {
            ...
            await this.#instantiate(graph, quad, template, this, stack)
        } else {
            for (const template of [...this.getElementsByTagName("TEMPLATE")]) {
                await this.#instantiate(graph, quad, template, template.parentNode, stack)
                template.remove()
            }
        }
    ...
    async #instantiate(graph, quad, template, parent, stack) {
        stack.push(quad)
        ...
        for (const child of [...template.content.cloneNode(true).childNodes]) {
            ...
            if (child instanceof Element) {
                await child.initialize(graph, quad, stack)
            }
        }
        stack.pop()
    }
